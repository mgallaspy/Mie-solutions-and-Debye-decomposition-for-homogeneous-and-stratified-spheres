function StratifiedSphere_ScatteredIntensity (orderP,n_med,ns_part,size_prms)
...
% Calculate partial wave coefficients, and then intensity at each point.
if orderP < 0
    [an, bn, ~, ~, ~, ~] = StratifiedSphere_PWC(size_prms, ns_part, n_med);
else
    [an, bn] = Debye_StratifiedSphere_PWC(size_prms, ns_part, n_med, orderP);
end


%[E_r,E_th,E_phi] = HomogeneousSphere_InternalField(rho,theta,phi,ind_max,n_medium,n_particle,size_prm,cn,dn,gnm_TE,gnm_TM);
[E_r, E_th, E_phi] = StratifiedSphere_ScatteredField(rho, theta, phi, n_med, size_prms, conj(an), conj(bn));
...
titlestr = sprintf('p=%i Onofri',orderP);
title(titlestr);
